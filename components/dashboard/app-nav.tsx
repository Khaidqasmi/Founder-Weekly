...
export function AppNav() {
  ...
  return (
    <nav className="border-b bg-[#221c4e] sticky top-0 z-50">
          ...
          {visibleItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? 'bg-gradient-to-r from-[#ec4899] to-[#d946ef] text-white font-medium' : 'text-white/60 hover:text-white'
              )}
            >
              {item.label}
            </Link>
      ...
      {open && (
        <div className="lg:hidden border-t px-4 py-3 space-y-1 bg-[#221c4e]">
          {visibleItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={cn('block px-3 py-2 rounded-md text-sm', pathname === item.href ? 'bg-gradient-to-r from-[#ec4899] to-[#d946ef] text-white font-medium' : 'text-white/60')}
