...
export function SiteHeader() {
  ...
  return (
    <nav className="border-b bg-white sticky top-0 z-50">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden border-t px-4 py-3 space-y-1 bg-white max-h-[70vh] overflow-y-auto">
          {links.map((item) => (
            <Link
              key={item.href}
              href={item.href}
          ...
          {!inApp && (
            <>
              <Link href="/privacy" className="block px-3 py-2 text-sm text-gray-600">Privacy</Link>
              <Link href="/terms" className="block px-3 py-2 text-sm text-gray-600">Terms</Link>
            </>
          )}
