...
import type { Remittance } from './client'
import { postexResponse } from './postex'

export function createCourierClient(credentials: Record<string, string>) {
function getStoredKey(key: string): string { return credentials[key] || '' }
...
async function postexProxy(resource: 'shipments' | 'remittances', token: string) {
  const response = await postexResponse(token, resource)
  const data = await response.json()
  if (!response.ok) throw new Error(data.error || 'PostEx request failed')
  return data
}

async function fetchTraxShipments(): Promise<Shipment[]> {
...
async function fetchLeopardsShipments(): Promise<Shipment[]> {
...
async function fetchCallCourierShipments(): Promise<Shipment[]> {
...
async function fetchPostExShipments(): Promise<Shipment[]> {
...
async function fetchAllShipments(): Promise<{ shipments: Shipment[]; errors: string[] }> {
...
async function fetchLeopardsRemittances(): Promise<Remittance[]> {
  const apiKey = getStoredKey('leopards_api_key')
  const apiPassword = getStoredKey('leopards_api_password')
  if (!apiKey) throw new Error('Leopards API key not configured')
...
async function fetchPostExRemittances(): Promise<Remittance[]> {
  const token = getStoredKey('postex_api_token')
  if (!token) throw new Error('PostEx API token not configured')
...
async function fetchTraxRemittances(): Promise<Remittance[]> {
  const apiKey = getStoredKey('trax_api_key')
  if (!apiKey) throw new Error('Trax API key not configured')
...
async function fetchAllRemittances(): Promise<{ remittances: Remittance[]; errors: string[] }> {
...
return { fetchAllShipments, fetchAllRemittances }
}
