...
export default function AdminPage() {
  ...
  if (loading) return <div className="min-h-screen bg-[#f5f3fb] p-8"><p>Loading admin...</p></div>
          ...
          <DialogContent>
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto">
              {noteModal.notes.map((n) => (
                <div key={n.id} className="bg-[#f5f3fb] p-3 rounded text-sm">
                  <p>{n.note}</p>
                  <p className="text-xs text-[#8d87b8] mt-1">{new Date(n.created_at).toLocaleString()}</p>
                </div>
              ))}
              {noteModal.notes.length === 0 && <p className="text-[#8d87b8] text-sm">No notes yet</p>}
            </div>
